...
def removeDonations():
    ...
    # checkNeedItem(removedItem)
# [END removing]


# def checkNeedItem(removedItem):
#     with foodBank.context():
#         count = removedItem.itemCount
# 
#     if count <= 10:
#         print("NEED MORE: " + removedItem.foodDesc + " in " + removedItem.bankName)
...
def fetchFoodNeeded(bank):
    with foodBank.context():
        query = FoodItem.query(FoodItem.bankName == bank, FoodItem.itemCount <= 10)
        items = [i.bankName + ' | ' + i.foodType + ' | ' + i.foodDesc + ' | ' + str(i.itemCount) for i in query]
